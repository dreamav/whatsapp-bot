...
class Client extends EventEmitter {
    ...
    async initialize() {
        ...
        if(this.options.session) {
            ...
            // Wait for QR Code
            await page.waitForSelector('._2d3Jz');
            const qr = await page.$eval('._1pw2F', node => node.getAttribute('data-ref'));
            const qr_img = await page.$eval('._1pw2F img', node => node.getAttribute('src'));
            //this.emit(Events.QR_RECEIVED, qr);
            this.emit(Events.QR_RECEIVED, qr_img);
